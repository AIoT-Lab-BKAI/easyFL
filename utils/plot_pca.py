...
def plot_updates_components(model, updates, peers_types, peers_id, epoch, proportion, attacked_class, dirty_rate, num_malicious, agg_algorithm, algorithm, type_noise):
    weight=False
    bias=False
    for i in range(3):
        if i ==1 :
        ...
        peers_typesDf = pd.DataFrame(data = peers_types,
                                    columns = ['target'])
        peers_idDf = pd.DataFrame(data = peers_id,
                                    columns = ['ID'])
        ...
        finalDf = pd.concat([grad_per_neuron, sum_gradsDf, peers_typesDf['target'],peers_idDf['ID']], axis = 1)
        path_csv = algorithm + '/' + agg_algorithm + '/' 'attacked_class_{}/dirty_rate_{}/type_noise_{}/proportion_{}/num_malicious_{}/csv/{}'.format(len(attacked_class),dirty_rate,type_noise,proportion*50,num_malicious,i)
