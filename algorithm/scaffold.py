...
class Server(BasicServer):
    ...
    def test(self, model=None, device=None):
        ...
        if self.test_data:
            ...
            if not Path(f"test/{self.option['algorithm']}").exists():
                os.makedirs(f"test/{self.option['algorithm']}")
            
            file_index = len(os.listdir(f"test/{self.option['algorithm']}"))
            np.savetxt(f"test/{self.option['algorithm']}/round_{file_index}.txt", cmtx, delimiter=",", fmt="%.2f")
