"""
This version diminish the effect of KD in time
Details:
    When a new model is created at server, we measures:
        d = norm(new_model, old_model)
    
    This measure does not imply anything of the performance of the model,
    however, we use it as indicator that the model has converged somehow
    
    And hence, we keep a record of these values (d) in a list:
        D = [d1, d2, d3 ...]
    
    When the model is considered converged when |d_(t + 1) - d_t| ~ 0
    let delta = |d_(t + 1) - d_t|
    if delta < epsilon:
        deactivate KDR
    else:
        activate KDR
...
class Server(MPBasicServer):
    ...
    def pack(self, client_id):
        return {
            ...
            "covergence_signal": self.convergence_signal()
        }
...
class Client(MPBasicClient):
    ...
    def train(self, model, convergence_signal, device):
    ...
    def reply(self, svr_pkg, device):
        model, convergence_signal = self.unpack(svr_pkg)
        loss = self.train_loss(model, device)
        self.train(model, convergence_signal, device)
        cpkg = self.pack(model, loss)
        return cpkg
    
    def unpack(self, received_pkg):
        # unpack the received package
        return received_pkg['model'], received_pkg['covergence_signal']