...
class Server(MPBasicServer):
    ...
    def update_Q_matrix(self, model_list, client_idx, t=None):
        ...
        self.Q_matrix = (self.freq_matrix * self.Q_matrix + new_similarity_matrix)/(self.freq_matrix + new_freq_matrix)
        self.Q_matrix[torch.isinf(self.Q_matrix)] = 0.0
        self.Q_matrix = torch.nan_to_num(self.Q_matrix, 0.0)
        
        self.freq_matrix += new_freq_matrix

        print(self.Q_matrix[new_freq_matrix > 0].reshape([self.clients_per_round, self.clients_per_round]))
        return
    ...
    def get_impact_factor(self, client_idx, t=None):
        ...
        Q_asterisk_mtx = (Q_asterisk_mtx > self.thr) * 1.0                      # Binary Clustering matrix
        np.savetxt(f"Cluster/cluster_{t}.txt", Q_asterisk_mtx.numpy(), fmt='%d')
        Q_asterisk_mtx = ((Q_asterisk_mtx.T @ Q_asterisk_mtx) > 0) * 1.0        # Transitive clustering matrix
        np.savetxt(f"Tran/transi_{t}.txt", Q_asterisk_mtx.numpy(), fmt='%d')
