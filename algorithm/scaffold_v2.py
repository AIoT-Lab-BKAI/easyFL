...
class Server(BasicServer):
    ...
    def iterate(self, t):
        ...
        data_vol_this_round = np.sum([self.client_vols[cid] for cid in self.selected_clients])
        # local training
        dys, dcs = self.communicate(self.selected_clients)
        if self.selected_clients == []: 
            return
        # aggregate
        impact_factors = [1.0 * self.client_vols[cid]/data_vol_this_round for cid in self.selected_clients]
        self.model, self.cg = self.aggregate(dys, dcs, p=impact_factors)
        return

    def aggregate(self, dys, dcs, p=[]):  # c_list is c_i^+
        dw = fmodule._model_average(dys, p)
        dc = fmodule._model_average(dcs, p)
