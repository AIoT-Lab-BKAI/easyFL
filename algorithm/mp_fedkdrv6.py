...
def KL_divergence(teacher_batch_input, student_batch_input, temp, device):
    """
    Compute the KL divergence of 2 batches of layers
    With distribution generated by temperatured-softmax
    Args:
    ...
    e_kernel_mtx_s = torch.exp(kernel_mtx_s/temp)
    kernel_mtx_s = e_kernel_mtx_s/torch.sum(e_kernel_mtx_s, dim=1, keepdim=True)
    ...
    e_kernel_mtx_t = torch.exp(kernel_mtx_t/temp)
    kernel_mtx_t = e_kernel_mtx_t/torch.sum(e_kernel_mtx_t, dim=1, keepdim=True)
...
class Client(MPBasicClient):
    ...
    def get_loss(self, model, src_model, data, device):
        ...
        kl_loss = KL_divergence(representation_t, representation_s, self.temperature,device)        # KL divergence
