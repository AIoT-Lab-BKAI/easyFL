"""
This version change the teacher
Details:
    The original KDR use the fused model as the teacher.
    In this version, a client i has its teacher by:
        teacher_i = server - impact_i * client_i
"""
...
class Client(MPBasicClient):
    ...
    def train(self, model, teacher, device):
        model = model.to(device)
        model.train()
        
        src_model = teacher.to(device)
    ...
    def reply(self, svr_pkg, device):
        ...
        teacher = copy.deepcopy(model - 1/10 * self.model if self.model is not None else model)
        
        self.train(model, teacher, device)
