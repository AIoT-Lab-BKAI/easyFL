...
def prob_transfer(student, teacher, transfer_loader, epochs=1, lr=0.001, layer_weights=(1,), kernel_parameters={}):
    ...
    for epoch in range(epochs):
        ...
        train_loss = 0
        qmi_loss = 0
        for inputs in transfer_loader:
            # Feed forward the network and update5r
            optimizer.zero_grad()

            inputs = inputs.cuda()

            teacher_feats = teacher.get_features(Variable(inputs))
            student_feats = student.get_features(Variable(inputs))
