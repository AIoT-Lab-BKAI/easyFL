"""
This version combine normal fedsdiv with trasitive property
of cosine similarity to boost the clustering proficiency
"""
...
class Server(BasicServer):
    ...
    def apply_transitive(self, input):
        for i in range(input.shape[0]):
            for j in range(input.shape[1]):
                for k in range(j + 1, input.shape[1]):
                    if input[i][j] == 1 and input[i][k] == 1:
                        input[j][k] = 1
                        input[k][j] = 1
        return input
    ...
    def get_impact_factor(self, client_idx, t=None):
        ...
        Q_asterisk_mtx = self.apply_transitive(Q_asterisk_mtx)
