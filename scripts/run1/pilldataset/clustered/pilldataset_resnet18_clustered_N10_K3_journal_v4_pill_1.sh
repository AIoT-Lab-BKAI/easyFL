...
GROUP="pilldataset_resnet18_clustered_N10_K3"
ALG="journal_v4_pill"
MODEL="resnet18"
STHR=0.9
EPS=0.5
...
NUM_GPUS=1
SERVER_GPU_ID=0
TASK="pilldataset_resnet18_clustered_N10_K3"
DATA_IDX_FILE="benchmark/pilldataset/data/syndata3"

cd easyFL

python main.py  --task ${TASK}  --model ${MODEL}  --algorithm ${ALG} --save_agent 1 --load_agent 0 --eps ${EPS} --sthr ${STHR} --wandb ${WANDB} --data_folder ${DATA_DIR}  --log_folder ${LOG_DIR}   --dataidx_path ${DATA_IDX_FILE}   --num_rounds ${ROUND} --num_epochs ${EPOCH_PER_ROUND} --proportion ${PROPOTION} --batch_size ${BATCH} --num_threads_per_gpu ${NUM_THRESH_PER_GPU}  --num_gpus ${NUM_GPUS} --server_gpu_id ${SERVER_GPU_ID} 